    ...
    def convert_to_batch_format(self, input_path, num_samples=200):
        ...
        if os.path.isdir(input_path):
            ...
            for fname in os.listdir(input_path):
                match = re.search(r'checkpoint-(\d+)$', fname)
                if match:
                    step_num = int(match.group(1))
                    if step_num % 50 != 0:
                        continue  # Skip non-50-multiples
                    full_path = os.path.join(input_path, fname, "model_responses.json")
                    try:
...
class CountdownAnalyzer(BaseAnalyzer):
    ...
    def create_analysis_prompts(self, numbers, target, completion):
        ...
        prompts = [
            ...
            f"""Scenario: {scenario}\nReasoning: {completion}\n\nGiven the following moral frameworks:\nA. Utilitarianism\nB. Deontology\nC. Justice\nD. Virtue Ethics\nE. Care Ethics\n\nReturn a number between 1 and 7 between each of the <utilitarianism>, <deontology>, <justice>, <virtue>, and <care> tags, where 1 means the reasoning does not invoke the framework and 7 means the reasoning strongly invokes the framework.\n\nYour response MUST follow this format:\n<utilitarianism>number</utilitarianism>\n<deontology>number</deontology>\n<justice>number</justice>\n<virtue>number</virtue>\n<care>number</care>\n""",
            # DEVELOPMENT_PROMPT
